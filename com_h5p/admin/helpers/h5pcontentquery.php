...
class H5PContentQuery {
	...
	public function __construct($fields, $offset = NULL, $limit = NULL, $order_by = NULL, $reverse_order = NULL, $filters = NULL) {
		$this->base_table = "#__h5p_contents hc";
		$this->valid_joins = array(
			'hl' => " LEFT JOIN #__h5p_libraries hl ON hl.id = hc.library_id",
			't' => " LEFT JOIN #__h5p_contents_tags ct ON ct.content_id = hc.id
			LEFT JOIN #__h5p_tags t ON ct.tag_id = t.id
			LEFT JOIN #__h5p_contents_tags ct2 ON ct2.content_id = hc.id"
		);
	...
	*/
	public function get_total() {
		...
		if (!empty($this->where_args)) {
			// We need to prep if we have args
			$query = call_user_func_array('sprintf', array_merge(array ( $query), $this->where_args));
		}
		$db->setQuery($query);
